...
def parse_args():
    ...
    p.add_argument("--ground-truth", default=str(REPO_ROOT / "scripts" / "bench_data" / "ground_truth.jsonl"))
    p.add_argument("--scenes-multiplier", type=int, default=2)
    p.add_argument("--timeout", type=float, default=180.0)
    p.add_argument("--output", default=str(REPO_ROOT / "scripts" / "bench_results" / "sweep.csv"))
    args = p.parse_args()
